...
export const login = async (data) => {
    ...
    cookieStore.set("BlackAshERP-token", token, {
        httpOnly: true,
        secure: "lax",
...
export const logout = async () => {
    const cookieStore = await cookies();
    cookieStore.delete("BlackAshERP-token");
    redirect("/login");
}

export const register = async (data) => {
    await connectDB();
    const { username, email, password, role, department, className, rollNo,subjectsTaught } = data;
    const user = await User.findOne({ email });
    ...
    const newUser = await User.create({ username, email, password: hashPassword, role , department,studentInfo:{className,rollNo},teacherInfo:{subjectsTaught }});
...
const getToken = async () => {
    const cookieStore = await cookies();
    const token = cookieStore.get("BlackAshERP-token");
